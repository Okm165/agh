...
int stack_push(double x){
	if (top >= STACK_SIZE) {printf("OVERFLOW "); return OVERFLOW;}
	stack[top] = x;
	top += 1;
	return OK;
}
...
void cbuff_push(int cli_nr){
	if (len >= CBUFF_SIZE) {printf("OVERFLOW "); return;}
	cbuff[(out+len)%CBUFF_SIZE] = cli_nr;
	len += 1;
}

void cbuff_pop(void) {
	if (len <= 0) {printf("UNDERFLOW "); return;}
	out = (out+1)%CBUFF_SIZE;
	len -= 1;
}

void cbuff_state(void){
	if (len == 0) {printf("EMPTY "); return;}
	for (int i = 0; i < len; i++){
