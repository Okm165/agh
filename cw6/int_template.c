...
double int_mid_rectangle(double x1, double x2, int n, f_1d f){
	...
	double step = (x2-x1)/n;
	for (int i = 0; i < n; i++) {
		integral += (f(x1 + i*step)+f(x1 + (i+1)*step))/2 * step;
	}
	return integral;
}
...
double int_trapezoid(double x1, double x2, int n, f_1d f){
	...
	double step = (x2-x1)/n;
	for (int i = 0; i < n; i++) {
		integral += min(f(x1 + i*step), f(x1 + (i+1)*step)) * step + (fabs(f(x1 + i*step) - f(x1 + (i+1)*step)) * step) / 2;
	}
	return integral;
}
...
double integral_2d(double x1, double x2, int nx, double y1, double y2, int ny,
	...
	for (int x = 0; x < nx; x++){
		for (int y = 0; y < ny; y++){
			if (boundary(x1 + stepx*x, y1 + stepy*y)){
				integral += f(x1 + stepx*x, y1 + stepy*y) * stepy * stepx;
			}
		}
	}
	return integral;
...
double integral_3d(double x1, double x2, int nx, double y1, double y2, int ny,
	...
	for (int x = 0; x < nx; x++){
		for (int y = 0; y < ny; y++){
			for(int z = 0; z < nz; z++){
				integral += f(x1+stepx*x, y1+stepy*y, z1+stepz*z) * stepx*  stepy * stepz;
			}
		}
	}
	return integral;
